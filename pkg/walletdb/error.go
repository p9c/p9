...
var (
	// ErrDbTypeRegistered is returned when two different database drivers attempt to register with the name database
	// type.
...
var (
	// ErrTxClosed is returned when attempting to commit or rollback a transaction that has already had one of those
	// operations performed.
	ErrTxClosed = errors.New("tx closed")
	// ErrTxNotWritable is returned when an operation that requires write access to the database is attempted against a
...
var (
	...
	// ErrIncompatibleValue is returned when trying create or delete a bucket on an existing non-bucket key or when
	// trying to create or delete a non-bucket key on an existing bucket key.
